...
export class RedInkClient {
   ...
   * 生成大纲
   */
  async generateOutline(topic: string, images?: Buffer[]): Promise<OutlineResponse> {
    if (!this.baseUrl) {
      await this.initialize();
    }

    console.log(`📝 正在生成大纲: ${topic.substring(0, 50)}...`);

    try {
      const response = await this.fetchWithRetry(`${this.baseUrl}/api/outline`, {
        method: 'POST',
        headers: {
      ...
      console.log(`✅ 大纲生成成功！共 ${data.pages?.length || 0} 页`);
      return data;
    } catch (error) {
